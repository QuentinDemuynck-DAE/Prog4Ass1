...
void dae::Walking::HandleInput(dae::GameObject& object, const Event& event)
{
	...
	if (event.id == make_sdbm_hash("collision_enter"))
	{
			...
			&& std::holds_alternative<void*>(event.args[1]))
		{
			...
			if (receiver->GetOwner().HasComponent<EnemyComponent>() && receiver->GetOwner().GetComponent<EnemyComponent>()->CanHit() && !m_PlayerComponent->IsInvincible()) //Collided with a falling ingredient so also fall down
			{
